package edu.illinois.techservices.sympa.demo;
...
/**
 * Demonstration of how to contact a Sympa server for email lists.
 * 
 * <P>
...
public class GetListsFromSympaDemo implements Runnable {
    ...
    private void getLists() {
        try {
            SOAPMessage smGetLists = MessageFactory.newInstance().createMessage();
            SOAPPart spGetLists = smGetLists.getSOAPPart();

            // Declare SOAP and sympa namespaces
            SOAPEnvelope senvGetLists = spGetLists.getEnvelope();
            addSympaNamespaceDeclarations(senvGetLists);

            // Create SOAP request body.
            SOAPBody sbGetLists = senvGetLists.getBody();
