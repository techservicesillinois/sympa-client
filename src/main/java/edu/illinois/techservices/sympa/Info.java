...
/**
 * How to execute:
 * mvn exec:java -Dexec.mainClass="edu.illinois.techservices.sympa.Info"
 * -Dsympa.email=$SYMPA_EMAIL -Dsympa.password=$SYMPA_PASSWORD
 * -Dsympa.url=$SYMPA_URL -Dexec.args="sampleListName"
 * 
 * 
 * <P>
...
public class Info implements Runnable {
   ...
   * a user's permissions for a specific email list. This also fetches
  ...
  private void invokeApiCall() {
    try {
      ...
      // API Call
      SOAPElement infoEl = reqBody.addChildElement("info", "ns", "urn:sympasoap");

      // Add parameters as child elements
      infoEl.addChildElement("list", "ns", "urn:sympasoap")
          .addTextNode(this.listName)
      ...
      reqHeaders.addHeader("SOAPAction", "urn:sympasoap#info"); // API Call
